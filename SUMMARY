...
The PC members should meet the SIGPLAN diversity policy http://www.sigplan.org/DiversityPolicy.htm. Sadly, achieving gender diversity in the HIW programme committee can be difficult due to the lack of qualified female candidates. If this is the case then note it explicitly on the workshops proposal.


5) Write up the Call For Talks (CFT) announcement, and discuss it with the PC. 

When choosing dates for proposal submission, notification etc, there are a few constraints:
 a) The HIW proposal deadline should be after Haskell Symposium notification. This is so people that get rejected from HS can still give a talk at HIW.
 b) Allow at least two weeks between HIW proposal submission and notification. The HIW PC will be spread out between timezones, so it takes a day for each 'round' in an email conversation. 
 c) The HIW notification date should be before the ICFP online registration system opens. Ask the ICFP workshops chair when this will be. Most workshop attendees will decide whether to come to HIW based on the abstracts on the website. The full programme plus abstracts should be there before they need to decide.


6) HIW has not yet had invited speakers, but this may be an option for later years. In 2011 we could give up to two invited speakers free registration to the workshop. This was done by supplying them with a "free registration code", obtained from the ICFP workshops chair. The speakers entered this code into the online registration system. 
...
7) Spam the CFT to the usual channels, haskell-cafe, reddit, facebook, twitter, FP users group lists etc.


8) Contact specific Haskell implementors you know might have a story to tell, and prod them to submit a talk proposal. You don't need to ask them to be "invited speakers", just ensure they submit a proposal.
...
10) The PC (chairs included) reads the proposals, ranks them, and decides who will talk at the workshop.
...
13) Respond to the ICFP workshops chair when they ask when you want your tea breaks.


14) Ask the ICFP workshops chair what the copyright status of the videos will be.
...
15) Attend the event, and delegate PC members (or other people) to chair each session.
